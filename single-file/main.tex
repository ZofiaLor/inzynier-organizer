...
(Streszczenie pracy – odpowiednie pole w systemie APD powinno zawierać kopię tego streszczenia.) \ksremark{to do}

\subsubsection*{Słowa kluczowe} 
(2-5 slow (fraz) kluczowych, oddzielonych przecinkami)\ksremark{to do}
...
\begin{otherlanguage}{british}
(Thesis abstract – to be copied into an appropriate field during an electronic submission – in English.)\ksremark{to do}
\end{otherlanguage}
\subsubsection*{Key words}  
\begin{otherlanguage}{british}
(2-5 keywords, separated by commas)\ksremark{to do}
\end{otherlanguage}
...
Obecnie wiele programów tworzonych jest w formie aplikacji przeglądarkowych. Aplikacje takie są niezależne od systemu operacyjnego użytkownika, \nocomma\  ani nie wymagają instalacji, dzięki czemu są łatwiejsze i wygodniejsze w obsłudze.
...
\begin{enumerate}
   \item Należy rozpakować pliki źródłowe. Można zrobić to w interfejsie graficznym, \nocomma\ lub wywołując w wybranym terminalu komendę:
   \begin{itemize}
...
Odpowiedzi: 
\begin{itemize}
	\item Poprawne zarejestrowanie: obiekt utworzonego użytkownika z pustym hasłem i rolą ROLE\_USER (jeżeli jest to pierwszy użytkownik w bazie danych, rola to ROLE\_ADMIN), kod 200 (OK) 
...
W przypadku back-endu należy zweryfikować, czy faktyczne odpowiedzi są zgodne z wymienionymi w podrozdziale ,,Lista Endpointów'' \ksremark{odwołanie poprzez etykietę \LaTeX a: \nameref{sec:lista-endpointow}}. Z kolei front-end powinien już na poziomie interfejsu w jak największym stopniu ograniczyć możliwość wprowadzenia błędnych danych.
...
Rejestrację przeprowadzono również przy pustej bazie danych. Pierwszy utworzony użytkownik otrzymał rolę ,,ROLE\_ADMIN'', każdy kolejny -- ,,ROLE\_USER''.
...
Odświeżanie ciasteczek: odświeżane było jedynie ciasteczko organizerJwt \ksremark{Tu i dalej: napisałbym w minted.} zamiast obydwu. 
...
\ksremark{To jest trochę długie. Napisałbym w pierwszym akapicie bardzo syntetycznie, co zostało zrobione. Takie streszczeni pracy na jeden akapit (może być trochę dłuższy).}

W ramach projektu zaprojektowano \ksremark{Najpierw napiszemy, że powstała aplikacja. Czym się cechuje. Takie krótkie powtórzenie ze wstępu. Tutaj też można napisać, co nowego wnosi ta aplikacja w stosunku do istniejących rozwiązań. Potem przejdziemy do szczegółów.} relacyjną bazę danych. Niektóre tabele odpowiadają występującym w temacie pojęciom -- użytkownik, plik, katalog. Inne, takie jak tabele przypisujące dostępy do elementów użytkownikom, bądź tabela zawierająca dane niezbędne do odświeżenia ciasteczek, są niezbędne do prawidłowego działania aplikacji, lecz niewidoczne dla jej użytkownika.

Implementację back-endu aplikacji utrzymano w konwencji frameworku Spring. Korzystając z mapowania obiektowo-relacyjnego tabele bazy danych zostały odwzorowane jako klasy języka Java. Dla każdej z nich zaimplementowano również klasy Repository \ksremark{tu i dalej: w minted}, Service oraz Controller, a dla większości także klasy DTO oraz Mapper. Utworzono również klasy kluczy złożonych, konieczne do implementacji klas w relacji N:M \ksremark{$N:M$}, oraz pomocnicze klasy wyjątków, zwiększające czytelność kodu odpowiedzialnego za sprawdzanie dostępu do plików i katalogów.
