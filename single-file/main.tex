...
\newcommand{\Title}{Webowy organizer zadań i wydarzeń}           % tytuł pracy po polsku
\newcommand{\TitleAlt}{Online task and event organizer}                     % thesis title in English
...
Zwraca wszystkie wysłane odczytane/nieodczytane powiadomienia aktualnie zalogowanego użytkownika, w~zależności od parametru \mintinline{sql}{read} (\mintinline{json}{true} -- odczytane, \mintinline{json}{false} -- nieodczytane).
...
W~odpowiedzi otrzymany został katalog o~ID 29, a~ID katalogu nadrzędnego (\mintinline{bash}{parent}) 26 -- zmiana nie zaszła.
...
Aplikację uruchomiono również w~przeglądarkach Microsoft Edge w~wersji 131.0 oraz Google Chrome w~wersji 131.0. Sprawdzono działanie operacji takich jak logowanie i~rejestracja, dodawanie i~udostępnianie plików, dodawanie powiadomień, głosowanie na terminy. Nie zauważono różnic w~działaniu aplikacji.
