...
\begin{itemize}
	...
	\item U.2. Administrator: posiada dostęp do wszystkich funkcjonalności użytkownika zwykłego, zarządza użytkownikami - \pauza\ nadaje i odbiera uprawnienia administratorskie, usuwa użytkowników.
	\item U.3. Użytkownik niezalogowany: loguje lub rejestruje się.
\end{itemize}	
\ksremark{Może byłoby lepiej tak jak niżej?}
\begin{description}
	\item [U.1] Użytkownik zwykły: edytuje profil, tworzy, przegląda, modyfikuje, usuwa, udostępnia elementy.
	\item [U.2] Administrator: posiada dostęp do wszystkich funkcjonalności użytkownika zwykłego, zarządza użytkownikami - \pauza\ nadaje i odbiera uprawnienia administratorskie, usuwa użytkowników.
	\item [U.3] Użytkownik niezalogowany: loguje lub rejestruje się.
...
Wymagania funkcjonalne przedstawiono na Rys. \ksremark{rys.} \ref{fig:uml-ucd}.

\begin{figure}
\centering
\includegraphics[height=\textheight]{./UML-UCD.png}
\caption{Diagram przypadków użycia aplikacji.}
\label{fig:uml-ucd}
\end{figure}
\ksremark{Rys. \ref{fig:uml-ucd}: Rysunek podzieliłbym na trzy osobne dla każdego aktora plus rysunek z relacjami pomiędzy aktorami.}

\section{Wymagania niefunkcjonalne}

Dla aplikacji wymagającej zalogowania przez użytkownika, służącej do przechowywania prywatnych informacji niezbędne jest zapewnienie bezpieczeństwa danych przed potencjalnym przechwyceniem. \ksremark{W jaki sposób zweryfikujemy, czy to wymaganie zostało spełnione?}

Ze względu na dużą ilość danych wprowadzanych przez użytkownika, powinna zostać zapewniona odporność na nieprawidłowe dane wejściowe. \ksremark{To jest wymaganie funkcjonalne.}

Istotna również jest intuicyjność interfejsu użytkownika. \ksremark{W jaki sposób zweryfikujemy, czy to wymaganie zostało spełnione?}

\section{Narzędzia}

Do implementacji back-endu wybrano framework Spring \ksremark{cytowanie dokumentacji, strony, …}, pozwalający m.in. na pisanie aplikacji z wykorzystaniem Java Persistence API (Spring Data JPA) oraz proste zaimplementowanie autentykacji i autoryzacji użytkowników (Spring Security). Za wybraniem tego frameworku zamiast np. Entity Framework \ksremark{cytowanie dokumentacji} przemawiała również posiadana już wiedza o REST API oraz chęć pogłębienia znajomości samego Springa.
Wspieraną przez Spring bazę danych MySQL \ksremark{cytowanie dokumentacji} oraz sam back-end postanowiono uruchomić w kontenerze Dockera \ksremark{cytowanie dokumentacji}.

Do wykonania front-endu posłużył framework Angular \ksremark{cytowanie dokumentacji}. Podobnie jak w przypadku Springa, za tym wyborem przemawiało pragnienie pogłębienia wstępnej wiedzy o frameworku. Subiektywną zaletą jest też, w przeciwieństwie do popularniejszego \footnote{\url{https://trends.stackoverflow.co/?tags=reactjs,vue.js,angular,svelte,angularjs,vuejs3}, dostęp 08.11.2024} Reacta \ksremark{cytowanie dokumentacji}, wyraźny rozdział na pliki HTML, CSS (SCSS) oraz TypeScript w ramach danego komponentu. 
Wykorzystano również komponenty Angular Material, dzięki którym w prosty sposób uzyskać można estetycznie wyglądający interfejs graficzny, pozwalając na skupienie się na samej logice programu. Angular Material zawiera większość potrzebnych w projekcie komponentów, jednak brakuje w nim komponentu do wizualizacji wydarzeń w kalendarzu. W tym celu wykorzystano otwartoźródłowy (Apache License 2.0) pakiet DayPilot \ksremark{cytowanie dokumentacji}.
