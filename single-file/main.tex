...
Do uruchomienia programu potrzebne są: JDK wersja 17, Docker \ksremark{jaka wersja?} (w systemie Linux zainstalować można sam silnik Docker, w systemie Windows potrzebna jest aplikacja Docker Desktop) oraz Node.js, wraz z NPM (wykorzystano wersję 20). Program wykorzystywać będzie porty: 3306 (baza danych), 8080 (back-end) i 4200 (front-end) -- należy więc zadbać, aby żadna inna aplikacja nie korzystała z nich podczas uruchamiania.
...
\end{itemize}
 ...
 Po poprawnym wykonaniu polecenia, wygenerowany zostanie katalog \mintinline{bash}{target}, zawierający plik z rozszerzeniem \mintinline{bash}{.jar}.
	...
	Aby zatrzymać kontener z poziomu terminala \comma\ należy najpierw znaleźć jego identyfikator używając polecenia 
...
\begin{enumerate}
	...
	Należy otworzyć go w wybranej przeglądarce, wklejając go w polu adresu, \nocomma\ bądź naciskając go lewym przyciskiem myszy, wciskając jednocześnie klawisz Ctrl.
...
\ksremark{Tutaj trzeba napisać kilka słów wprowadzenia / wstępu. Teraz zaczynamy od razu z grubej rury.}
...
Odświeża JWT odpowiadający za autoryzację użytkownika, jeżeli \mintinline{json}{refreshToken} \ksremark{Chyba minted trochę się gubi w przypadku JSON-a.} zapisany w ciasteczkach nie stracił ważności.
...
\begin{itemize}
	...
	\item \mintinline{json}{refreshToken} stracił ważność: kod 403 (\english{Forbidden}) 
	\item Nie znaleziono \mintinline{json}{refreshToken} w bazie danych: kod 404 (\english{Not Found})
	\item \mintinline{json}{refreshToken} w ciasteczku jest pusty lub \mintinline{json}{null}: kod 400 (\english{Bad Request})
\end{itemize}
...
Parametry brane pod uwagę: name, textContent, isFinished, deadline \ksremark{$\gets$ w minted}.
...
Zwraca wszystkie wysłane odczytane/nieodczytane powiadomienia aktualnie zalogowanego użytkownika, w zależności od parametru \mintinline{json}{read} (\mintinline{json}{true} - \ksremark{--} odczytane, \mintinline{json}{false} - \ksremark{--} nieodczytane).
...
\begin{itemize}
	...
	\item Brak dostępu do pliku: kod 403 (\english{Forbidden})
\end{itemize}
...
Przy rejestracji, zgodnie z założeniami, otrzymano kod 400 \comma\ gdy w ciele zapytania brakowało hasła lub nazwy użytkownika, oraz kod 403 \comma\ gdy wpisano nazwę użytkownika występującą już w bazie danych. Dla poprawnych danych:
\begin{minted}{json}
{
	"username": "test",
	"password": "password",
...
Element można przenieść tylko do istniejącego katalogu, należącego do tego samego użytkownika, przez tego użytkownika. Ignorowane są przypadki, gdy podejmowana jest próba przeniesienia katalogu bazowego, \nocomma\ bądź ustawienia katalogu nadrzędnego na ten sam katalog lub katalog podrzędny. Taka sytuacja mogłaby skutkować nieskończoną pętlą podczas sprawdzania dostępu, a także błędem bazy danych przy usuwaniu.
\ksremark{Tu choć jedno zdanie wprowadzające przykład.}
\begin{minted}{json}
{
	"id": 29,
	"parent": 29
...
W przypadku katalogu bazowego, \nocomma\ próba usunięcia skutkuje kodem błędu 403.
...
\ksremark{Piszę tutaj, ale odnosi się to do tekstu wyżej i niżej. Mamy w tym rozdziale wiele przypadków testowych. Warto by je wizualnie wydzielić. Tutaj robię tak przykładowo do «przetestowano zmianę hasła». Czy tak jest lepiej?}

\paragraph{Zmiana hasła}
%Przetestowano zmianę hasła. 
Jeżeli nie podane zostanie stare lub nowe hasło \comma\ otrzymywany jest kod 400. Dla niepoprawnego starego hasła zwracany jest kod 403. Zmieniono hasło w poprawny sposób i spróbowano zalogować się przy pomocy starego i nowego hasła -- operacja przebiegła pomyślnie dla hasła nowego. 
...
\ksremark{W tym podrozdziale też podbiłbym wizualnie poszczególne przypadki.}
Podczas testowania front-endu, \nocomma\ tam, gdzie nie zaznaczono inaczej, korzystano z przeglądarki Mozilla Firefox w wersji 133.0. Wysyłane do back-endu zapytania weryfikowano przy pomocy inspektora przeglądarki.

Po przejściu na adres \mintinline{bash}{http://localhost:4200/} w przeglądarce, \nocomma\ pokazana zostaje strona powitalna. Użytkownik jest niezalogowany -- w górnym pasku nawigacyjnym dostępny jest przycisk ,,Log In''. 

Sprawdzono zabezpieczenia przed próbą dostępu do nieudostępnionych użytkownikowi niezalogowanemu zasobów. Przejście na stronę \mintinline{bash}{http://localhost:4200/file/1} przeładowuje stronę, nie zostaje jednak wysłane żadne zapytanie, a widok strony nie zmienia się. Próba wejścia na stronę profilu \mintinline{bash}{http://localhost:4200/profile} skutkuje wyświetleniem komunikatu ,,Please log in to view your profile'' \ksremark{\english{Please log in to view your profile} i dalej analogicznie.}. Próba wejścia na dostępną administratorowi stronę \mintinline{bash}{http://localhost:4200/admin/users} pokazuje komunikat ,,This page is accessible for administrators only''. Przejście na niezdefiniowany adres, na przykład \mintinline{bash}{http://localhost:4200/wrong} przekierowuje na prostą stronę ,,Page Not Found'', zawierającą odnośnik do strony głównej. 
...
Aby zweryfikować automatyczne odświeżanie ciasteczek, tymczasowo zmieniono ich czas ważności w back-endzie na 1 minutę dla \mintinline{json}{organizerJwt} i 3 minuty dla \mintinline{json}{organizerRefreshJwt} \ksremark{Chyba się trzeba będzie zlitować nad minted i dać mu «bash» zamiast «json».}. W inspektorze przeglądarki sprawdzono, że co jedną minutę cyklicznie wywoływane zapytanie wysyłające powiadomienia zwraca kod 403 i wysyłana jest prośba o odświeżenie ciasteczka. Następnie zamknięto kartę aplikacji i ponownie otwarto ją po ponad 3 minutach. Użytkownik został automatycznie wylogowany.
...
\ksremark{Choć zdanie wprowadzenia, np. «Testowanie ujawniło szereg błędów w aplikacji» czy coś może mniej ostrego.}
Autentykacja: podczas logowania generowane są dwa ciasteczka: \mintinline{bash}{organizerJwt} \ksremark{Proszę sprawdzić, gdzieś wyżej w pracy ciasteczko jest napisane bez minted. Trzeba je tam też zapisać w minted.} oraz \mintinline{bash}{organizerRefreshJwt}. Drugie z ciasteczek służy do odświeżania pierwszego, powinno więc mieć dłuższy czas ważności. Przez przypadek do obydwu ciasteczek przypisywano tę samą wartość \mintinline{java}{maxAge}.
...
Panel dostępu do elementów: obiekty \mintinline{java}{AccessFile} lub \mintinline{java}{AccessDirectory} oraz obiekty \mintinline{java}{User} były przechowywane w osobnych tablicach. Obiekty użytkowników były w innej kolejności w tablicy, \nocomma\ niż obiekty dostępów -- błędnie założono, że dodawanie użytkowników iterując po obiektach dostępów spowoduje, że kolejność zostanie zachowana. W wyniku tego błędu zmiany w przydzielonych dostępach następowały nie dla tych użytkowników, których nazwy były wyświetlone w liście w interfejsie graficznym panelu. Błąd naprawiono przechowując pary obiektów we wspólnej tablicy.
...
Do implementacji dziedziczenia klas \mintinline{java}{Event}, \mintinline{java}{Note} oraz \mintinline{java}{Task} po klasie \mintinline{java}{File} w bazie danych wykorzystano metodę \english{Single Table Inheritance}, przez co w bazie odpowiada im wszystkim jedna tabela. Dzięki zastosowaniu dziedziczenia, \nocomma\ operacje odczytu oraz usunięcia wierszy wystarczyło zaimplementować raz – dla klasy bazowej. Operacje tworzenia oraz aktualizacji wierszy musiały jednak być napisane osobno dla każdej klasy.
